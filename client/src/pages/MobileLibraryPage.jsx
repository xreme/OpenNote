...
function DetailView({ item, collectionId, onBack, onRefresh }) {
  ...
  const [content, setContent] = useState(
    isNote ? item.content : (item._summary?.content ?? null),
  );
  ...
  return (
    ...
    >
      ...
      {showInfo && (
        <SourceInfoSheet video={item} onClose={() => setShowInfo(false)} />
      )}
...
export default function MobileLibraryPage() {
  ...
  useEffect(() => {
    const hasProcessing = videos.some(
      (v) => !["completed", "error"].includes(v.status),
    );
  ...
  const standaloneNotes = notes.filter(
    (n) => !videoCleanNames.some((name) => n.filename.startsWith(name)),
  );
  ...
  return (
    ...
    >
      ...
      {infoVideo && (
        <SourceInfoSheet video={infoVideo} onClose={() => setInfoVideo(null)} />
      )}
      {showAddSheet && (
        <AddSourceSheet
          collectionId={collectionId}
